...
export default function AdminBusiness() {
  ...
  // Sous-onglet Parrainage
  const [parrainageSousOnglet,    setParrainageSousOnglet]    = useState("Liens"); // "Liens" | "Inscriptions"
  const [parrainageInscriptions,  setParrainageInscriptions]  = useState([]);
  const [parrainageInscrTotal,    setParrainageInscrTotal]    = useState(0);
  const [parrainageInscrSearch,   setParrainageInscrSearch]   = useState("");
  const [parrainageInscrStatus,   setParrainageInscrStatus]   = useState(""); // "" | "actif" | "attente" | "suspendu"
  const [parrainageInscrPage,     setParrainageInscrPage]     = useState(1);
  const INSCR_LIMIT = 30;
  ...
  const loadParrainageInscriptions = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ page: parrainageInscrPage, limit: INSCR_LIMIT });
      if (parrainageInscrSearch) params.set("search", parrainageInscrSearch);
      if (parrainageInscrStatus) params.set("status", parrainageInscrStatus);
      const d = await get(`/api/business/admin/parrainage-inscriptions?${params}`);
      setParrainageInscriptions(d.data?.clients || d.clients || []);
      setParrainageInscrTotal(
        d.data?.pagination?.total ??
        d.pagination?.total ??
        (d.data?.clients || d.clients || []).length
      );
    ...
    setLoading(false);
  }, [get, parrainageInscrPage, parrainageInscrSearch, parrainageInscrStatus]);
  ...
  useEffect(() => {
    ...
    if (tab === "Parrainage" && parrainageSousOnglet === "Inscriptions") loadParrainageInscriptions();
    if (tab === "Sce Technique" && sceUnlocked) loadScePayments();
  }, [
    tab, sceUnlocked, parrainageSousOnglet,
    loadMembers, loadCommissions, loadBonusPool,
    loadDemandesCommission, loadClients, loadCollectes,
    loadParrainages, loadParrainageInscriptions, loadScePayments,
  ]);
      ...
      {tab === "Parrainage" && (
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {["Liens", "Inscriptions"].map(so => (
              <button
                key={so}
                onClick={() => setParrainageSousOnglet(so)}
                style={{
                  padding: "7px 18px", borderRadius: 8, border: "1.5px solid",
                  cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: parrainageSousOnglet === so ? 700 : 500,
                  background:  parrainageSousOnglet === so ? "#7C3AED" : "#fff",
                  color:       parrainageSousOnglet === so ? "#fff" : "#64748B",
                  borderColor: parrainageSousOnglet === so ? "#7C3AED" : "#E2E8F0",
                }}
              >
                {so === "Liens" ? "🔗 Liens" : "👤 Inscriptions"}
              </button>
            ))}
          </div>

          {/* ── Sous-onglet INSCRIPTIONS ── */}
          {parrainageSousOnglet === "Inscriptions" && (
            <div>
              <div style={s.filters}>
                <input
                  style={s.input}
                  placeholder="🔍 Nom client, email, parrain…"
                  value={parrainageInscrSearch}
                  onChange={e => { setParrainageInscrSearch(e.target.value); setParrainageInscrPage(1); }}
                />
                <select style={s.select} value={parrainageInscrStatus} onChange={e => { setParrainageInscrStatus(e.target.value); setParrainageInscrPage(1); }}>
                  <option value="">Tous les statuts</option>
                  <option value="actif">Actif</option>
                  <option value="attente">En attente</option>
                  <option value="suspendu">Suspendu</option>
                ...
                <button style={s.btnSecondary} onClick={loadParrainageInscriptions}>↻ Rafraîchir</button>
              </div>
              ...
              {loading ? <Spinner /> : (
                <div style={s.table}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#F8FAFC" }}>
                        {["Client","Parrain","Rôle parrain","Token","Date inscription","Statut"].map(h => (
                          <th key={h} style={s.th}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {parrainageInscriptions.map(insc => {
                        const stCfg = CLIENT_STATUS_MAP[insc.status] || { bg: "#F1F5F9", color: "#64748B", label: insc.status || "—" };
                        const roleC = ROLE_COLORS[insc.parrain_role] || {};
                        return (
                          <tr key={insc.id} style={s.tr}>
                            {/* Client */}
                            <td style={s.td}>
                              <div style={s.memberName}>{insc.client_name || insc.name || "—"}</div>
                              <div style={s.memberSub}>{insc.client_email || insc.email || ""}</div>
                              {(insc.client_phone || insc.phone) && (
                                <div style={s.memberSub}>{insc.client_phone || insc.phone}</div>
                              )}
                            </td>
                            {/* Parrain */}
                            <td style={s.td}>
                              <div style={s.memberName}>{insc.parrain_name || "—"}</div>
                              <div style={s.memberSub}>{insc.parrain_email || ""}</div>
                            </td>
                            {/* Rôle parrain */}
                            <td style={s.td}>
                              {insc.parrain_role ? (
                                <span style={{ ...s.badge, background: roleC.bg || "#F1F5F9", color: roleC.color || "#64748B", border: `1px solid ${roleC.border || "#E2E8F0"}` }}>
                                  {insc.parrain_role}
                                </span>
                              ) : <span style={{ color: "#CBD5E1" }}>—</span>}
                            </td>
                            ...
                            <td style={s.td}>
                              <span style={{ fontFamily: "monospace", fontSize: 12, fontWeight: 700, color: "#7C3AED" }}>
                                {insc.parrainage_token || "—"}
                              </span>
                            </td>
                            {/* Date */}
                            <td style={{ ...s.td, fontSize: 12, color: "#64748B", whiteSpace: "nowrap" }}>
                              {insc.created_at
                                ? new Date(insc.created_at).toLocaleDateString("fr-FR", { day: "2-digit", month: "short", year: "numeric" })
                                : "—"}
                            </td>
                            {/* Statut */}
                            <td style={s.td}>
                              <span style={{ ...s.badge, background: stCfg.bg, color: stCfg.color }}>
                                {stCfg.label}
                              </span>
                            </td>
                          </tr>
                      ...
                      {parrainageInscriptions.length === 0 && (
                        <tr>
                          <td colSpan={6} style={{ textAlign: "center", padding: 48, color: "#94A3B8" }}>
                            <div style={{ fontSize: 40, marginBottom: 12 }}>👤</div>
                            <p style={{ fontWeight: 700 }}>Aucune inscription via parrainage trouvée</p>
                            <p style={{ fontSize: 12 }}>Les clients inscrits via un lien de parrainage apparaîtront ici</p>
                          </td>
                        </tr>
                      )}
                    </tbody>
              ...
              {/* Pagination */}
              <div style={s.pagination}>
                <button style={s.btnSecondary} disabled={parrainageInscrPage === 1} onClick={() => setParrainageInscrPage(p => p - 1)}>← Préc.</button>
                <span style={{ fontSize: 13, color: "#64748B" }}>
                  Page {parrainageInscrPage} · {parrainageInscrTotal} inscription{parrainageInscrTotal !== 1 ? "s" : ""}
                </span>
                <button
                  style={s.btnSecondary}
                  disabled={parrainageInscrPage * INSCR_LIMIT >= parrainageInscrTotal}
                  onClick={() => setParrainageInscrPage(p => p + 1)}
                >
                  Suiv. →
                </button>
              </div>
          ...
          {/* ── Sous-onglet LIENS (contenu existant) ── */}
          {parrainageSousOnglet === "Liens" && (<div>
          {!loading && (
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
